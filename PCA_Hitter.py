...
#2017 Hitters
f = open('player_record/hitter17.csv', 'rt', encoding='UTF8')
lines = csv.reader(f)
record17 = [line for line in lines]
f.close()

#make a player_list having Players
player_list = [Player(record[1], record[2], 'Hitter') for record in record17[1:]]
...
for player in player_list:
    for record in record17:
        if player.name == record[1]:
            player.update(Record_Hitter(record[0], record[1], record[2], record[3], record[4], record[5], record[6], record[7], record[8], record[9], record[10], record[11], record[12], record[13], 2017))


#2018 Hitters
f = open('player_record/hitter18.csv', 'rt', encoding='UTF8')
lines = csv.reader(f)
